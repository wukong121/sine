...
class Model(object):
    ...
    def _xent_loss_weight(self, user, seq_multi, user_eb_augs):
        emb_dim = self.dim
        loss = tf.nn.sampled_softmax_loss(
            weights=self.output_item2(),
    ...
    def _calculate_cl_loss(self, batch_sample_one, batch_sample_two):
...
class Model_SINE_SSL(Model):
    def __init__(self, n_mid, embedding_dim, hidden_size, batch_size, seq_len, topic_num, category_num, alpha, beta,
        ...
        self.seq_augs_multi = [self.sequence_encode_cpt(item_aug_emb, self.nbr_mask) for item_aug_emb in self.item_aug_embs]  # [(none,4,128), (none,4,128)]
        self.user_eb_augs = self.split_multi_seq(self.seq_augs_multi)
        # self.user_eb_augs = [self.labeled_attention(seq_aug_multi) for seq_aug_multi in self.seq_augs_multi]

        self._xent_loss_weight(self.user_eb, self.seq_multi,  self.user_eb_augs)
        self.summary_loss()

    def split_multi_seq(self, seq_augs_multi):
        seq1 = tf.split(seq_augs_multi[0], num_or_size_splits=self.category_num, axis=1)
        seq2 = tf.split(seq_augs_multi[1], num_or_size_splits=self.category_num, axis=1)
        splits = seq1 + seq2
        seq_augs = [tf.squeeze(seq, axis=1) for seq in splits]
        return seq_augs
